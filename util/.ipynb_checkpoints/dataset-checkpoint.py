...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None, use_split_coco=False, \
                        transform=None, transform1=None, transform_tri=None, mode='train', ann_type='mask', \
    ...
    def __getitem__(self, index):
        ...
        for k in range(self.shot):  
            ...
            support_label1 = support_label
            support_label1[ignore_pix[0],ignore_pix[1]] = 255
            if self.data_set == 'pascal':
                support_padding_label = np.zeros_like(support_label1)
                support_padding_label[support_label1==255] = 255
            else:
                support_padding_label = np.zeros_like(support_label1)
            ...
            support_label_list1.append(support_label1)
        ...
        if self.transform is not None:
            ...
            iamge1, label1, padding_mask = self.transform1(image1, label1, padding_mask)
            for k in range(self.shot):
                support_image_list[k], support_label_list[k] = self.transform(support_image_list_ori[k], support_label_list_ori[k])
                _, _, support_padding_list[k] = self.transform1(support_image_list_ori[k], support_label_list1[k], support_padding_list[k])
        ...
        # Return
        if self.mode == 'train':
            return image, label, label_b, s_x, s_y, padding_mask, s_ey, subcls_list
        elif self.mode == 'val':
            return image, label, label_b, s_x, s_y, padding_mask, s_ey, subcls_list, raw_label, raw_label_b
        elif self.mode == 'demo':
            total_image_list = support_image_list_ori.copy()
            total_image_list.append(raw_image)            
            return image, label, label_b, s_x, s_y, padding_mask, s_ey, subcls_list, total_image_list, support_label_list_ori, support_label_list_ori_mask, raw_label, raw_label_b          
