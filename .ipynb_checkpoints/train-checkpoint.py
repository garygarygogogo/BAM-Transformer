...
from util import transform, transform1, transform_tri, config
...
def get_model(args):
    ...
    optimizer, transformer_optimizer = model.get_optim(model, args, LR=args.base_lr)
...
# ----------------------  DATASET  ----------------------
    ...
    train_transform1 = transform1.Compose([
        transform1.RandScale([args.scale_min, args.scale_max]),
        transform1.RandRotate([args.rotate_min, args.rotate_max], padding=mean, ignore_label=args.padding_label),
        transform1.RandomGaussianBlur(),
        transform1.RandomHorizontalFlip(),
        transform1.Crop([args.train_h, args.train_w], crop_type='rand', padding=mean, ignore_label=args.padding_label),
        transform1.ToTensor(),
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco':
        train_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root, base_data_root=args.base_data_root, data_list=args.train_list, \
                                    transform=train_transform, transform1=train_transform1, transform_tri=train_transform_tri, mode='train', \
    ...
    if args.evaluate:
        if args.resized_val:
            val_transform = transform.Compose([
                ...
                transform.ToTensor(),
                transform.Normalize(mean=mean, std=std)])
            val_transform1 = transform1.Compose([
                transform1.Resize(size=args.val_size),
                transform1.ToTensor(),
                transform1.Normalize(mean=mean, std=std)])
        ...
        if args.data_set == 'pascal' or args.data_set == 'coco':
            val_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root, base_data_root=args.base_data_root, data_list=args.val_list, \
                                    transform1=val_transform1, transform=val_transform, transform_tri=val_transform_tri, mode='val', \
                                    data_set=args.data_set, use_split_coco=args.use_split_coco)                                   
...
# ----------------------  TRAINVAL  ----------------------
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # ----------------------  TRAIN  ----------------------
        loss_train, mIoU_train, mAcc_train, allAcc_train = train(train_loader, val_loader, model, optimizer, transformer_optimizer, epoch)
...
def train(train_loader, val_loader, model, optimizer, transformer_optimizer, epoch):
    global best_miou, best_FBiou, best_piou, best_epoch, keep_epoch, val_num
    global best_miou_m, best_miou_b, best_FBiou_m
    ...
    for i, (input, target, target_b, s_input, s_mask, padding_mask, s_padding_mask, subcls) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss1, aux_loss2 = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b, cat_idx=subcls, padding_mask=padding_mask, s_padding_mask=s_padding_mask)
...
def validate(val_loader, model):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, padding_mask, s_padding_mask, subcls, ori_label, ori_label_b) in enumerate(val_loader):
            ...
            start_time = time.time()
            output, meta_out, base_out = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b, cat_idx=subcls, padding_mask=padding_mask, s_padding_mask=s_padding_mask)
